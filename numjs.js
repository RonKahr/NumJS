(function(window){
    ...
    function define_numjs() {
        ...
        numjs.RMatrix.prototype.LUPSolve = function () {
            /*
            Given a system of equations such as:
            x +  y = 7
            x + 2y = 11

            Solve for x and y using LUP decomposition and then forward and backward substitution.

            More generally we want to solve Ax=b.  We will start with a single matri where in the above A=[[1,1],[1,2]] and b=[7,11]
            */
            //get the A matrix
            var colsA = this.columnCount() - 1;
            var rowsA = this.rowCount();
            var A = new numjs.RMatrix(colsA, colsA);
            for (var k = 0; k < colsA; k++) {
                for (var l = 0; l < rowsA; l++) {
                    A.set(k, l, this.get(k, l));
                }
            }
            //get b vector
            var b = this.getColumnVector(this.columnCount() - 1);

            //get the LU decomposition matrix
            var lup = A.LUPDecomposition();
            
            /*
            * Given LU,P and b solve for x.
            * */
            var n = lup.LU.columnCount() - 1;
            var x = new numjs.RVector(rowsA);
            var y = [];
            var suml = 0;
            var sumu = 0;
            var lij = 0;
            ...
            * */
            ...
            for (var i = n; i >= 0; i--) {
                sumu = 0;
                for (var j = i + 1; j <= n; j++) {
                    sumu = sumu + (lup.LU.get(i, j) * x.get(j));
                }
                xi = (y[i] - sumu) / lup.LU.get(i, i);
                x.set(i, xi);
            }
        ...
        //Initialize an instance of the RVector class from an array
        numjs.convertArrayToRVector = function (data) {
            if (data.length < 1)
                throw new Error("The dimension of the array must be greater than 0.");
            var size = data.length;

            var vector = new numjs.RVector(size);
        ...
        //Utility functions
        numjs.compareArrays = function (arry1, arry2, delta) {
            //Compare two arrays of equal size to see if the terms match
            //within a delta value
            return true;
        }
